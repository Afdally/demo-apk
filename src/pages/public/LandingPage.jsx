...
const STEPS = [
  { num: '1', title: 'Pilih paket foto', desc: 'Pilih paket sesuai kebutuhanmu — solo, couple, keluarga, atau maternity. Harga tertera transparan.' },
  { num: '2', title: 'Tambah layanan ekstra', desc: 'Print foto, background tambahan, atau extra waktu bisa ditambahkan langsung dengan estimasi harga.' },
  { num: '3', title: 'Pilih tanggal & jam', desc: 'Kalender menampilkan slot yang masih tersedia secara real-time. Tidak ada lagi double booking.' },
  { num: '4', title: 'Konfirmasi via WhatsApp', desc: 'Pesan WA terisi otomatis dengan detail booking kamu. Tinggal kirim ke admin untuk konfirmasi final.' },
]

const TESTIMONIALS = [
  { text: 'Booking-nya gampang banget. Langsung tau slot mana yang kosong, tidak perlu nunggu reply WA berjam-jam. Hasilnya juga keren!', name: 'Sari Rahayu', role: 'Pelanggan · Paket Couple', initials: 'SR', color: '#C1121F' },
  { text: 'Sebagai pemilik studio, sekarang semua booking bisa saya pantau dari satu dashboard. Tidak ada lagi double booking dan laporan manual!', name: 'Budi Hartono', role: 'Admin · Luminary Studio', initials: 'BH', color: '#1D3557' },
  { text: 'Awalnya ragu, tapi ternyata proses booking cuma butuh 2 menit. Langsung dapat konfirmasi dan tidak perlu daftar akun dulu. Recommended!', name: 'Dewi Anggraini', role: 'Pelanggan · Paket Family', initials: 'DA', color: '#2B4875' },
]
      ...
      <div className="foto-mosaic">
        {MOSAIC_PHOTOS.map((photo) => (
          <div key={photo.cls} className={`mosaic-block ${photo.cls}`}>
            <img src={photo.src} alt={photo.alt} className="mosaic-img" style={{ objectPosition: photo.position }} />
          </div>
        ))}
      </div>
    </section>
...
export default function LandingPage() {
  ...
  useEffect(() => {
    // Simulasi loading seperti fetch API
    const timer = setTimeout(() => {
      setPackages(MOCK_PACKAGES.filter(p => p.is_active))
      setLoading(false)
    }, 600)
