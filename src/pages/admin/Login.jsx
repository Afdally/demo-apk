...
// Kredensial demo untuk Vercel demo
const DEMO_CREDENTIALS = {
  email: 'admin@studiobook.id',
  password: 'admin123',
}
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (email === DEMO_CREDENTIALS.email && password === DEMO_CREDENTIALS.password) {
      setAdmin({ name: 'Admin', email }, 'mock-token-demo')
      navigate('/admin/dashboard')
    } else {
      setError('Email atau password salah. Gunakan: admin@studiobook.id / admin123')
    }
    setLoading(false)
  }
    ...
    <div className="admin-login-page">
      ...
      <div className="login-right">
        <div className="login-form-container">
          ...
          {/* Demo hint */}
          <div style={{ padding: '12px 16px', background: 'rgba(13,27,42,0.04)', borderRadius: '10px', fontSize: '13px', marginBottom: '24px', border: '1px solid rgba(13,27,42,0.08)' }}>
            <strong>Demo login:</strong> admin@studiobook.id / admin123
          </div>

          <form onSubmit={handleLogin}>
            {error && (
              <div style={{
                padding: '12px', background: 'rgba(193, 18, 31, 0.1)', color: 'var(--secondary)',
                borderRadius: '8px', fontSize: '13px', marginBottom: '20px', border: '1px solid rgba(193, 18, 31, 0.2)'
              }}>
                {error}
              </div>
            )}
