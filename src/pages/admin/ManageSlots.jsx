...
export default function ManageSlots() {
  const [hours, setHours] = useState(MOCK_SCHEDULE.hours)
  const [blocked, setBlocked] = useState(MOCK_SCHEDULE.blocked)
  ...
  const handleSaveAll = async () => {
    ...
    setSaving(false)
    alert('Jadwal berhasil disimpan! (mode demo)')
  }

  const handleSaveBlock = () => {
    if (!blockDate || !blockDesc) { alert('Tanggal dan Keterangan wajib diisi.'); return }
    const timeRange = blockStart && blockEnd ? `${blockStart} - ${blockEnd}` : 'Seharian'
    setBlocked((prev) => [...prev, { id: Date.now(), date: blockDate, timeRange, desc: blockDesc }])
    setIsModalOpen(false)
    setBlockDate(''); setBlockStart(''); setBlockEnd(''); setBlockDesc('')
  }

  const removeBlocked = (id) => setBlocked((prev) => prev.filter((b) => b.id !== id))
  ...
  return (
    <div style={{ paddingBottom: '80px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <button onClick={handleSaveAll} className="admin-btn" style={{ margin: 0, width: 'auto', padding: '10px 20px' }} disabled={saving}>
        ...
        <div className="slots-panel">
          <div>
            ...
            <p style={{ fontSize: '13px', color: 'var(--text-45)', marginBottom: '16px' }}>
              Atur jam buka, tutup, dan durasi slot untuk setiap harinya.
            </p>
          </div>
              ...
              <div key={h.day} className={`hour-row ${!h.active ? 'inactive' : ''}`} style={{ flexWrap: 'wrap' }}>
                ...
                {h.active && (
                  <div style={{ display: 'flex', gap: '12px', width: '100%', alignItems: 'center' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <select value={h.duration} onChange={(e) => updateTime(i, 'duration', Number(e.target.value))} className="time-field" style={{ width: '100%', padding: '8px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div className="slots-panel">
            ...
            <div className="blocked-list">
              {blocked.length === 0 ? (
                <div style={{ color: 'var(--text-45)', fontSize: '13px', fontStyle: 'italic' }}>Tidak ada tanggal yang diblokir.</div>
              ) : blocked.map((b) => (
                <div key={b.id} className="blocked-item" style={{ alignItems: 'flex-start' }}>
                  <div className="blocked-info">
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: '12px', background: 'var(--primary-mid)', padding: '2px 6px', borderRadius: '4px' }}>{b.timeRange}</span>
                    </div>
          ...
          <div className="slots-panel">
            <div><span className="panel-label">Simulasi Slot</span><h2 className="panel-title">Preview Jadwal</h2></div>
            <div style={{ marginTop: '16px', marginBottom: '16px' }}>
              <select className="time-field" style={{ width: '100%', padding: '10px' }} value={previewDay} onChange={(e) => setPreviewDay(e.target.value)}>
                {hours.map((h) => <option key={h.day} value={h.day}>Preview Hari {h.day}</option>)}
              </select>
            </div>
            {hours.find((h) => h.day === previewDay)?.active ? (
              previewSlots.length > 0 ? (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', maxHeight: '250px', overflowY: 'auto' }}>
                  {previewSlots.map((slot, idx) => (
                    <div key={idx} style={{ background: 'var(--surface)', border: '1px solid var(--white-06)', padding: '8px', borderRadius: '6px', textAlign: 'center', fontSize: '13px' }}>
                      {slot}
                    </div>
                  ))}
                </div>
              ) : (
                <div style={{ textAlign: 'center', padding: '24px', color: 'var(--text-45)', background: 'var(--primary-mid)', borderRadius: '8px' }}>Durasi terlalu panjang.</div>
              )
            ) : (
              <div style={{ textAlign: 'center', padding: '24px', color: 'var(--text-45)', background: 'var(--primary-mid)', borderRadius: '8px' }}>Studio tutup pada hari {previewDay}.</div>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="slots-panel" style={{ width: '100%', maxWidth: '400px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                <input type="text" value={blockDesc} onChange={(e) => setBlockDesc(e.target.value)} className="time-field" style={{ width: '100%' }} placeholder="Contoh: Maintenance, Private Event" />
              </div>
            </div>
            <div style={{ display: 'flex', gap: '12px', marginTop: '24px', justifyContent: 'flex-end' }}>
              <button onClick={() => { setIsModalOpen(false); setBlockDate(''); setBlockStart(''); setBlockEnd(''); setBlockDesc('') }} style={{ padding: '10px 16px', background: 'transparent', border: 'none', color: 'var(--text-60)', cursor: 'pointer' }}>Batal</button>
              <button onClick={handleSaveBlock} style={{ padding: '10px 16px', background: 'var(--text)', color: 'var(--primary)', borderRadius: '6px', border: 'none', cursor: 'pointer', fontWeight: 500 }}>Simpan Blokir</button>
            </div>
          </div>
        </div>
      )}
